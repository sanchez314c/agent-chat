...
echo %BLUE%[%TIME%]%NC% Launching compiled AgentCHAT (Windows)...
...
if exist "dist\win-unpacked\AgentCHAT.exe" (
    set "APP_PATH=dist\win-unpacked\AgentCHAT.exe"
    echo %BLUE%[%TIME%]%NC% Found unpacked executable: AgentCHAT.exe
    goto :found
)

REM Check for ia32 unpacked
if exist "dist\win-ia32-unpacked\AgentCHAT.exe" (
    set "APP_PATH=dist\win-ia32-unpacked\AgentCHAT.exe"
    echo %BLUE%[%TIME%]%NC% Found 32-bit unpacked executable: AgentCHAT.exe
    goto :found
)
