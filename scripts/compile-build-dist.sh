...
bloat_check() {
    ...
    if [ -f "package.json" ]; then
        DEV_DEPS=$(node -e "const p=require('./package.json'); console.log(Object.keys(p.devDependencies||{}).length + Object.keys(p.dependencies||{}).length)" 2>/dev/null || echo "0")
...
npx tsc --noEmit || print_warning "TypeScript compilation warnings detected"
...
if [ "$PLATFORM" = "all" ] || [ "$PLATFORM" = "win" ]; then
    if ls dist/*.exe 1>/dev/null 2>&1 && ! ls dist/*.msi 1>/dev/null 2>&1; then
...
if [ -d "dist" ]; then
    ...
    if [ $MAC_COUNT -gt 0 ]; then
        ...
        find dist -name "*-mac.zip" -type f | while read -r zip; do
...
print_info "  Linux:   ./run-source-linux.sh"
print_info "  macOS:   ./run-source-mac.sh"
print_info "  Windows: run-source-windows.bat"