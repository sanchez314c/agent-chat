...
export class AgentManager {
  ...
  private prepareMessages(agent: AgentConfig, conversationMessages: Message[]): Message[] {
    ...
    for (const msg of contextMessages) {
      // [H7] Filter out ALL system messages by role (not just persona by id) to prevent doubles
      if (msg.role === MessageRole.SYSTEM) {
        // System messages are handled by the persona injection above; skip all of them
        continue
      } else if (msg.role === MessageRole.OPERATOR) {
        // Operator messages are only visible to the first responding agent
        // They appear as user messages but with special handling
        // [H6] Determine first agent dynamically instead of hardcoded 'agent1'
        const firstAgentId = conversationMessages.find(m => m.agentId)?.agentId
  ...
  private fixRoleAlternation(messages: Message[]): Message[] {
    ...
    for (const msg of messages) {
      if (msg.role === lastRole) {
        // If same role as previous, merge by creating a new object (never mutate originals from React state)
        if (fixed.length > 0) {
          fixed[fixed.length - 1] = {
            ...fixed[fixed.length - 1],
            content: fixed[fixed.length - 1].content + '\n\n' + msg.content
          }
        } else {
          fixed.push({ ...msg })
        }
